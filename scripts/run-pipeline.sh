...
EOF
    ...
    for env_name in AGYN_MODEL_ID AGYN_AGENT_IMAGE AGYN_AGENT_INIT_IMAGE AGYN_API_TOKEN AGYN_ORGANIZATION_ID EGRESS_DATAPLANE_BASE_URL EGRESS_DIRECT_BYPASS_URL EGRESS_DIRECT_BYPASS_HEADER EGRESS_DIRECT_BYPASS_HEADER_VALUE ZITI_SIDECAR_IMAGE; do
      env_value=${!env_name:-}
      if [ -n "$env_value" ]; then
