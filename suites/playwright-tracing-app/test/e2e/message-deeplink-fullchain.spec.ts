...
test.describe('message deep link full chain', { tag: ['@svc_tracing_app', '@svc_agents_orchestrator'] }, () => {
  test('resolves message to run and renders timeline', async ({ page }) => {
    ...
    if (run.supportsMessageLink) {
      await expect(page).toHaveURL(runUrlPattern, { timeout: 60000 });
    } else {
      await page.goto(`/${run.organizationId}/runs/${run.runId}`);
      await expect(page).toHaveURL(runUrlPattern, { timeout: 60000 });
    }
